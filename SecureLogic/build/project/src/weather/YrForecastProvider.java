...
public class YrForecastProvider extends ForecastProvider {
	...
	class YrForecastParser extends DefaultHandler {
		...
		public List<WeatherObject> getForecasts() {
			//We have set the cutoff to 5 days in the future, that is 6 INCLUDING today..
			//Thats is correct if todays forecast aint one of them, which is the case after around 2300...
			//But if its < 2300 we need to remove the last day, we can have only 5...
			
			int firstDay = forecasts.get(0).DayOfMonth;
						
			Date toDay = new Date(System.currentTimeMillis());
			Calendar dayCal = Calendar.getInstance();
			dayCal.setTime(toDay);
			int todayDayOfMonth = dayCal.get(Calendar.DAY_OF_MONTH);
			if (todayDayOfMonth > firstDay) {
				dayCal.add(Calendar.MONTH, 1);
			}
			dayCal.set(Calendar.DAY_OF_MONTH, firstDay);
			
			dayCal.add(Calendar.DAY_OF_YEAR, 4);
			int lastDay = dayCal.get(Calendar.DAY_OF_MONTH);
			
			List<WeatherObject> retList = new ArrayList<>();
			for(WeatherObject wO : forecasts ) {
				if (wO.DayOfMonth >= firstDay || wO.DayOfMonth <= lastDay) {
					retList.add(wO);
				}
			}
		...
		private YrForecastParser() {
			...
			Calendar dayCal = Calendar.getInstance();
			dayCal.setTime(toDay);
			dayCal.add(Calendar.DAY_OF_YEAR, 5);
